function jBPMFormsAPI() {
    ...
    var sendRequest = function(config, method) {
        ...
        ajaxHandler.ajaxReq.open(method, config.url, true, config.username, config.password);
        ajaxHandler.ajaxReq.withCredentials = true;
        ajaxHandler.ajaxReq.send();
    }

    this.showStartProcessForm = function(hostUrl, deploymentId, processId, responseDiv, successCallback, errorCallback, _username, _password) {
                    ...
                    if (status && status.length > 0 && status[0].childNodes.length > 0) {
                        ...
                        if (status == 'SUCCESS') {
                            ...
                            if (formURL && formURL.length > 0 && formURL[0].childNodes.length > 0) {
                                ...
                                this.formURL = hostUrl + '/preauth.jsp?username=' + _username + '&password=' + _password + '&redirectTo=' + encodeURIComponent(this.formURL);
    ...
    this.showTaskForm = function (hostUrl, taskId, responseDiv, successCallback, errorCallback, _username, _password) {
